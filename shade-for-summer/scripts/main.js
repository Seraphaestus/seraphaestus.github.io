...
const targetImageShaders = {
	...
	"1": {caption: "Alright, all packed and ready to go! Wish me luck.", stickers: {friend:"2024/stickers/silhouette.png", van:"2024/stickers/van_side.png"}, targetCode: "const vec3 sky_color = vec3(194.0, 237.0, 255.0) / 255.0;const vec3 road_color = vec3(199.0) / 255.0;const vec3 building_color = vec3(255.0, 250.0, 194.0) / 255.0;const float top_cutoff = 0.85;const float bottom_cutoff = 0.15;void main(){COLOR.a=1.0;vec2 coord=(COORD-vec2(0.5))*vec2(0.8,1.15);COLOR.rgb=building_color;if(RATIO.y>top_cutoff){COLOR.rgb=sky_color;}if(RATIO.y<bottom_cutoff){COLOR.rgb=road_color;}}"},
	"2": {caption: "Isn't she a beaut? I'm glad she'll finally see some proper use.", stickers: {van:"2024/stickers/van_front.png"}, targetCode: "const vec3 sky_color = vec3(194.0, 237.0, 255.0) / 255.0;const vec3 road_color = vec3(199.0) / 255.0;const vec3 building_color = vec3(255.0, 250.0, 194.0) / 255.0;const float top_cutoff = 0.94;const float bottom_cutoff = 0.07;void main(){COLOR.a=1.0;vec2 coord=(COORD-vec2(0.5))*vec2(0.8,1.15);if(abs(coord.x)>abs(coord.y)){COLOR.rgb=building_color;if(coord.x>-0.5&&coord.x<0.5){COLOR.rgb*=0.975;}if(RATIO.y>top_cutoff){COLOR.rgb=sky_color;}if(RATIO.y<bottom_cutoff){COLOR.rgb=road_color;}}else{COLOR.rgb=mix(road_color,sky_color,(sign(coord.y)+1.0)*0.5);}}"},
